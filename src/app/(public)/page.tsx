...
export default async function HomePage() {
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-10 px-6 py-16 text-center">
      <div>
        <h1 className="font-display text-3xl font-bold text-foreground">Select your chapter.</h1>
      </div>
      ...
      <div className="grid w-full max-w-3xl gap-4 sm:grid-cols-2">
        {orgs.map((org) => (
          <form key={org.id} action={continueWithOrg.bind(null, org.id, `/org/${org.slug}`)}>
            <button
              type="submit"
              className="flex w-full flex-col items-center gap-4 rounded-2xl border border-border bg-surface p-10 text-center transition-colors hover:border-signal hover:bg-surface-raised"
            >
              {org.logoUrl ? (
                ...
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-full text-xl font-bold ${org.primaryColor ? "text-on-brand" : "bg-signal text-on-signal"}`}
                  style={org.primaryColor ? { backgroundColor: org.primaryColor } : undefined}
                >
                  {org.shortName.slice(0, 2).toUpperCase()}
                </div>
              )}
              <span className="font-display text-lg font-bold text-foreground">{org.name}</span>
            </button>
          </form>
        ))}
      </div>
